...
net_rgb = None
if FUSION == "EARLY":
    net_fus = cv2.dnn.readNet('yolov3_training_last_f.weights', 'yolov3_testing/yolov3_testing_f.cfg')
if FUSION == "LATE":
    net_therm = cv2.dnn.readNet('yolov3_training_last_t.weights', 'yolov3_testing/yolov3_testing_t.cfg')
    net_rgb = cv2.dnn.readNet('yolov3_training_last_c.weights', 'yolov3_testing/yolov3_testing_c.cfg')
...
for i in range(200, 300): # można zmieniać range do 518
    ...
    if FUSION == "LATE":
        ...
        for pair in paired_boxes:
            r1 = Rect1[pair[0]]
            r2 = Rect2[pair[1]]
            avg_r = [int((r1[0] + r2[0]) / 2), int((r1[1] + r2[1]) / 2), int((r1[2] + r2[2]) / 2),
                     int((r1[3] + r2[3]) / 2)]
            boxes.append(avg_r)
